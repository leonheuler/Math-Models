...
fig6 = figure(6);
clf('reset'); title('Currents');
global i1p i2p i3p
subplot(3,1,1);
[t, tau1] = getpoints(tau1p);
i1p = animatedline(); grid on;
addpoints(i1p, t, Km*tau1);
subplot(3,1,2);
[t, tau2] = getpoints(tau2p);
i2p = animatedline(); grid on;
addpoints(i2p, t, Km*tau2);
subplot(3,1,3);
[t, tau3] = getpoints(tau3p);
i3p = animatedline(); grid on;
addpoints(i3p, t, Km*tau3);
