function Test(i) 
%% Model config
    global hm1 hm2 hm3 m1 m2 m3 ml L1 L2 L3 L4 L5 g Lh Lt 
    ...
    Ch =  30000; Bh = Ch/25;
    Ct =  30000; Bt = Ct/100;
    Chj = 30000; Bhj = Chj/25;

    %% Simulation config
    q0 = [pi/2-0.01 pi/2+0.01 pi/3 0 0 0 ].'; % stand up
    tbeg = 0;
    tend = 2*pi;  
    dt = 0.1;
    tspan = tbeg:dt:tend; 
    ...
    subplot(3,2,5);
    Hp =  animatedline(); title('Hnorm');
    subplot(3,2,6);
    gammaHp = animatedline(); title('Hip spring angle');
    subplot(3,2,3);
    HJp = animatedline(); title('HJnorm');
    subplot(3,2,4);
    gammaHJp = animatedline(); title('Hip Joint spring magnitude');
    subplot(3,2,1);
    Tp =  animatedline(); title('Tnorm');
    subplot(3,2,2);
    gammaTp =  animatedline(); title('Torso spring angle');

    fig7 = figure(7);
    if (flag ==1) clf('reset'); end
    global tau1loadp tau2loadp tau3loadp
    subplot(3,1,1);
    tau1loadp = animatedline(); title('tau1load');
    subplot(3,1,2);
    tau2loadp = animatedline(); title('tau2load');
    subplot(3,1,3);
    tau3loadp = animatedline(); title('tau3load');
    %% Solver config
    fig1 = figure(1);
    if (flag == 1) clf('reset'); end
    ...
    eO5x = L1*cos(eq1) + L2*cos(eq2) + L4*cos(eq3+pi/2 + pi/8);
    eO5y = L1*sin(eq1) + L2*sin(eq2) + L4*sin(eq3+pi/2 + pi/8);
    eO6x = eO5x + L5*cos(eq3 + pi/8);
    eO6y = eO5y + L5*sin(eq3 + pi/8); 
    ...
    for i=1:last-1
        ...
        addpoints(exo, eO5x(i),eO5y(i));
        addpoints(exo, eO6x(i),eO6y(i));
        ...
        addpoints(foot, 0, 0);
        addpoints(foot, 0.24, -0.1);
        addpoints(foot, -0.05, -0.1);
        addpoints(foot, 0, 0);
...
%         global m1 m2 m3 ml L1 L2 L3 g Lh Lt 
%         global Ch Chj Ct Bh Bhj Bt
%         global tau1p tau2p tau3p
%         global tau1extp tau2extp tau3extp
...
%                   -pi/6*sin(t);
%                   pi/12*sin(t) ];
%         q_h = [pi/2-0.01 pi/2+0.01 pi/4].';
%         qdot_h = [ 0 0 0 ].';
        q_h = [3*pi/8 + pi/8*cos(t);
               5*pi/8 - pi/8*cos(t);
               pi/4   + pi/12*cos(t)
                ];
         qdot_h = [ -pi/8*sin(t);   
                    pi/8*sin(t);
                    -pi/12*sin(t) ];
%         q_h = [ Poli5(0,pi,pi/6,pi/2-0.01,0,0,0,0,t);
%                 Poli5(0,pi,pi/2+pi/6,pi/2+0.01,0,0,0,0,t);
%                 Poli5(0,pi,pi/6,pi/2-0.01,0,0,0,0,t) ];
%         
%      qdot_h = [ Poli5dot(0,pi,pi/6,pi/2-0.01,0,0,0,0,t);
%                 Poli5dot(0,pi,pi/2+pi/6,pi/2+0.01,0,0,0,0,t);
%                 Poli5dot(0,pi,pi/6,pi/2-0.01,0,0,0,0,t) ];     
        ...
        % stand up
        tau_react = [  Tnorm*L1*sin(gammaT-q_h(1)) + HJnorm*L1*sin(gammaHJ-q_h(1)) + Hnorm*L1*sin(gammaH-q_h(1));
                       Tnorm*L2*sin(gammaT-q_h(2)) + HJnorm*L2*sin(gammaHJ-q_h(2)) + Hnorm*Lh*sin(gammaH-q_h(2));
                       Tnorm*Lt*sin(gammaT-q_h(3)) ];
        
        tau_load = [ -Flnorm*L1*sin(gammaL-q_h(1));
                     -Flnorm*L2*sin(gammaL-q_h(2)); 
                     -Flnorm*L3*sin(gammaL-q_h(3)) ];
                 
        tau_ext = tau_react + tau_load;
    ...
    %     dx = [ x(4); x(5); x(6); -M(q) \ N(q,qdot)] + [zeros(3,1); M(q) \ (tau + tau_ext)]+ [zeros(3,1); -M(q) \ tau_d];
        ...
        addpoints(tau1extp, t, tau_react(1));
        addpoints(tau2extp, t, tau_react(2));
        addpoints(tau3extp, t, tau_react(3));

        addpoints(tau1loadp, t, tau_load(1));
        addpoints(tau2loadp, t, tau_load(2));
        addpoints(tau3loadp, t, tau_load(3));
