%% Model config
global hm1 hm2 hm3 m1 m2 m3 ml L1 L2 L3 g Lh Lt 
...
Ch =  1000; Bh = Ch/100;
Ct =  10000; Bt = Ct/100;
Chj = 1000; Bhj = Chj/100;

%% Simulation config
q0 = [pi/2-0.01 pi/2+0.01 pi/4 0 0 0 ].'; % stand up
tbeg = 0;
tend = pi;
dt = 0.01;
tspan = tbeg:dt:tend; 
...
%               pi/12*sin(t) ];
    ...
    % stand up
    tau_ext = [ -Flnorm*L1*sin(gammaT-q_h(1)) + Tnorm*L1*sin(gammaT-q_h(1)) + HJnorm*L1*sin(gammaHJ-q_h(1)) + Hnorm*L1*sin(gammaH-q_h(1));
                -Flnorm*L2*sin(gammaT-q_h(2)) + Tnorm*L2*sin(gammaT-q_h(2)) + HJnorm*L2*sin(gammaHJ-q_h(2)) + Hnorm*Lh*sin(gammaH-q_h(2));
                -Flnorm*L3*sin(gammaT-q_h(3)) + Tnorm*Lt*sin(gammaT-q_h(3)) ];
